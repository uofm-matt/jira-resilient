...
- **`search_seek` auto-recovers from two more `after_key` failure modes**
  ...
  - `"Operator '>' cannot be applied to moved issue key 'X'."` — fires
    ...
    Hit live on jte/OMDS (OMDS-3422) one hour after the 0.2.1 deploy.
...
### Fixed
- **DMDHMSM-class hub timeouts no longer block delta sync.** Before 0.1.3, a
...
- **JQL timezone bug.** JIRA Server's JQL parser interprets bare `"YYYY-MM-DD HH:MM"`
  ...
  Eastern-time JIRA, every delta sync's filter was shifted ~4 hours forward —
